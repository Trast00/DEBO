...
const Tender = ({tender, isHidden=false, isSaved=false, isViewed=false, userUuid, updateHiddenTenderById, updateSaveTenderById, showModal, isPremuim}) => {
  const {
    _id,
    title,
    description,
  ...
  const handleViewed = (tenderId) => {
    ...
    fetch(`/api/users/${userUuid}/preferences/tenders/viewed`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    <div className="tender" data-tender-id={_id} data-viewed={isViewedTender}>
      ...
      <div className="tender-buttons d-flex justify-content-between">
        ...
        {isPremuim && (
          <>
          { showModal && (
            <button className="show button-default" onClick={_ => {
                handleViewed(_id)
                showModal(tender)
              }}>Voir plus
              </button>
